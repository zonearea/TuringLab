...
4. **Kenar durumları:** n=1 doğrudan kabul (`q1l`). n=256: taşma sonrası `q_rv` ile `q_reject`. Geçersiz sembol (ör. başta `0`) için geçiş yok — motor `no_transition` döner (isteğe bağlı genişletme: `q0`’da `0`→`q_reject`).
